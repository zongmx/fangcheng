...
<div data-role="content" class="detail_content gray_bg">
	<div class="pc_offer_detail pc_main_w overflow myapplycs">
		...
		<?php foreach($result as $key => $val){?>
				<div  class="offer_detail_top  bgfff posr" style="border-bottom:1px solid #e4e4e4;padding:10px 10px;">
        <ul>
			...
			<?php } else { ?>
				<?php if (strtotime($val['csInfo']['cs']['expire_at'] )> strtotime("-1 day") ) {
					if ($val['csInfo']['cs']['status'] == 1 && $val['csInfo']['cs']['result'] == 1) {
						...
						<div class="hide" id="weixinshare_<?php echo $val['demand_id']?>"
								weixin-share-detail_{value['brand_id']}
								wxTitle="<?php if($val['demand_type'] == 2){ ?>招商：<?php }else{?>拓展：<?php } echo $val['demandInfo']['cs_name']; ?>"
								wxDesc=""
								wxLink="<?php echo C('WEB_URL')."/ucenter/myapplycsinfo/csid/{$val['demand_id']}";?>"
								wxImgUrl="<?php echo "http://m.fangcheng.cn/upload/brand/src/0/22/22184.png";?>"
								></div>
						     <div class="posa" onclick='WeiXinShareBtn("<?php echo $val['demand_id'];?>");' style="bottom:10px;right:10px;">
...
function WeiXinShareBtn(demand_id){
	var weixinShareDetail = $('#weixinshare_'+demand_id);
