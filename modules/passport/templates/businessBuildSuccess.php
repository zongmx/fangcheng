...
<input type="hidden" name="Doaction" value="businessBuild">
	<div class="reg_set2">
    <div class="info clearfix">
        ...
        <!--  -->
        <!-- 负责的区域 -->
				<div class="clearfix margin-top-20 brand_detail_area">
			<div class="clearfix pos">
				<div class=" add_change add_change_border area">
					<span class="custom_add_change">
						<span class="custom_add_name">负责的细分区域</span>

    					<span class="custom_add_icon">+</span>
    				</span>&nbsp;&nbsp;
					<span class="add_change_for add_change_for_noshow add_change_area">没有适合的区域？</span>
				</div>
			</div>
		</div>
		<!-- 负责区域 测试数据  passport_area 表-->
		 <input type="hidden" name="passport_area_area_id[]" value="20">
		 <input type="hidden" name="passport_area_area_id[]" value="30">
		<script>
            $(function(){
                $(".brand_detail_area .custom_add_icon").click(function(){
                    $(".brand_detail_area").city({"city_name":'area_name',"city_id":'area_id',"plugName":"brand_detail_area1","singleCheck":false});//city_name:当前调用的input城市的名称，city_id:当前调用的input的城市的id
                });

                $(".add_change_area").click(function(){
                    if($(this).hasClass("add_change_for_noshow")){
                        $(this).removeClass("add_change_for_noshow");
                        $(".add_change_for_box").removeClass("hide");
                    }else{
                        $(this).addClass("add_change_for_noshow");
                        $(".add_change_for_box").addClass("hide");
                    }
                });
            });
        </script>
		<div class="clearfix add_change_for_box hide margin-top-20">
			<div class="form-group clearfix">
                <div class="form_left">
                    <div class="area">
                        <input type="text" name="" class="form-control text" value="" placeholder="（如全国、华北大区等）">
                    </div>
                </div>
                <!-- 汉字填写  测试数据 passport 表 -->
                <input type="hidden" name='passport_job_area' value="华北大区 测试数据">
                <div class="tip emailTip hide"><span class='tipMsg'>如全国、华北大区等</span></div>
            </div>
		</div>
        <!--  -->
        <!-- 负责的业态 -->
        <div class="clearfix margin-top-20">
            ...
            <!-- 业态测试数据 -->
            <input type="hidden" name='category_id_one[]' value="1">
            <input type="hidden" name='category_ids[]' value="4,5,6,9">
            <input type="hidden" name='category_id_one[]' value="2">
            <input type="hidden" name='category_ids[]' value="1,3,5,7,8">
