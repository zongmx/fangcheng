 ...
 * 用户的所有动作行为
 */
class DemandActions extends Action
{
     ...
     * 品牌发布需求
     */
    public function executeBrandneed(Application $application, Request $request)
    {
        ...
        if ($_SESSION['userinfo']['passport_type'] == 2 || $_SESSION['userinfo']['passport_type'] == 3){
           $this->headerTo('/demand/businessneed');
        }
        // 获得当前人要展示的信息
        $this->setView('brandneed');
        $this->resultArray = [];
    ...
    public function executeBusinessneed(Application $application, Request $request)
    {
        ...
        if ($_SESSION['userinfo']['passport_type'] == 1) {
           $this->headerTo('/demand/brandneed');
        }
